...
| 실기기에서 노트북 API 사용 | 빌드 전에 노트북 IP로 설정 후 빌드. 예: `VITE_API_URL=http://172.30.1.71:8081` |

- **값이 없을 때** 기본값은 `http://172.30.1.71:8081` 입니다. 사용하는 PC IP에 맞게 `page/.env` 또는 `page/.env.production` 에 넣거나, 빌드 시 환경 변수로 지정하세요.
...
| Android 반영 | `npx cap sync` |
| Android Studio에서 실행 | `npx cap open android` → Run → 기기 선택 |
